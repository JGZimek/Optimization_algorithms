...
void process_dataset(const std::string& filename, const std::string& output_directory, int dataset_number) {
    ...
    auto initial_tasks = TaskLoader::read_tasks_sa(filename, data_id);
...
void process_datasets(const std::string& filename, const std::string& output_directory, int start, int end) {
    ...
    std::string command = "python3 ../python/plot_sa_results.py " + output_directory;
...
int main() {
    try {
        const std::string filename = "../data/data_sa.txt"; // relative path to the data file
